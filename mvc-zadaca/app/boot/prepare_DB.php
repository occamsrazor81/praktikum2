...
if( $has_tables )
{
	exit( 'Tablice dz2_users / dz2_projects / dz2_members veÄ‡ postoje. ObriĹˇite ih pa probajte ponovno.' );
}
...
// Ubaci neke projekte unutra (ovo nije baĹˇ pametno ovako raditi, preko hardcodiranih id-eva usera)
try
{
	$st = $db->prepare( 'INSERT INTO dz2_projects(id_user, title, abstract, number_of_members, status) VALUES (:id, :t, :a, :no, :st)' );

	$st->execute( array( 'id' => 1, 't' => 'Igra - Go', 'a' => 'Implementirat Ä‡emo Go u kojem korisnik moĹľe igrati protiv AI-ja.', 'no' => 3, 'st' => 'open' ) ); // mirko
	$st->execute( array( 'id' => 2, 't' => 'Novi Fejsbuk', 'a' => 'U ovom projektu napravit Ä‡emo novi Fejsbuk koji Ä‡e joĹˇ skupljati joĹˇ viĹˇe informacija o svojim korisnicima i koji Ä‡e
                joĹˇ ÄŤeĹˇÄ‡e i joĹˇ sluÄŤajnije curiti te informacije treÄ‡im stranama.', 'no' => '5', 'st' => 'open' ) ); // ana
	$st->execute( array( 'id' => 3, 't' => 'Web-aplikacija za recepte', 'a' => 'Napravit Ä‡emo aplikaciju sa svim receptima iz kuharice moje bake.', 'no' => 4, 'st' => 'open' ) ); // maja
 	$st->execute( array( 'id' => 1, 't' => 'Kao Amazon, ali bolje', 'a' => 'Jeff Bezos nema pojma, naĹˇ Amazon Ä‡e biti puno bolji.', 'no' => 2, 'st' => 'closed' ) ); // mirko
	$st->execute( array( 'id' => 4, 't' => 'Projekt za RP2', 'a' => 'VeÄ‡ Ä‡emo neĹˇto smisliti, prvo idemo okupiti tim.', 'no' => 4, 'st' => 'open' ) ); // slavko
}
...
// Ubaci neke ÄŤlanove unutra (ovo nije baĹˇ pametno ovako raditi, preko hardcodiranih id-eva usera i projekata)
try
{

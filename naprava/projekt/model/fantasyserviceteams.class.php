...
//
  ...
  function getPlayerStatsByPlayerIdAndWeek($id_plr, $week)
  {
    try
    {
      ...
      $st = $db->prepare('SELECT id, id_player, FGM, FGA, FG_PERC, FTM, FTA, FT_PERC,
        3PTM, PTS, REB, AST, ST, BLK, TOV, week, day from project_player_stats
         where id_player in (select id from project_players
           where id=:id_plr and week=:week)');

      $st->execute(array('id_plr' => $id_plr, 'week' => $week));
...
//promijeniti na usere umjesto timova u insert into
  function makeFirstWeeklyMatchUp($id_league, $id_user1, $id_user2, $week)
  {
    try
    {

      $db = DB::getConnection();
      $st = $db->prepare('INSERT into project_weekly_matchups
        (id_league, id_user1, id_user2, week)
      values(:id_league, :id_user1, :id_user2, :week)');

      $st->execute(array('id_league' => $id_league, 'id_user1' => $id_user1,
    'id_user2' => $id_user2, 'week' => $week ));
